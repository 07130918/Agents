...
name: serena
description: |
  `/serena`、`serena`、`serena-dev`、または「Serena方式」が明示された時に、アプリ開発、バグ調査、設計、実装、リファクタ、レビュー、最適化を構造化して進める。debug/design/implement/review、-q/-d/-c/-s/-r/-t などの深度指定に対応し、Serena MCP tools が未接続でも通常の検索、読解、編集で代替する。単純なバグ調査、PR作成、CI復旧など専用 skill が明らかな依頼では、その専用 skill を優先する。
