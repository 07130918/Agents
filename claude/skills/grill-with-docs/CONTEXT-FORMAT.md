...
- **実装詳細を入れない**: DB カラム名、API route、UI コンポーネント名、内部 helper 名は、それ自体がドメイン専門家の言葉として意味を持つ場合だけ入れる
- **解決前の仮説を入れない**: canonical term、定義、既存用語との関係、コードとの矛盾が解決し、ユーザが明示同意した後にだけ更新する
...
複数コンテキストで用語を追加する時は、次を明記する:

- その用語を所有する context
- 同じ名前の語が別 context で違う意味を持つか
- context をまたぐ関係が `CONTEXT-MAP.md` に反映されているか
- 関係変更が ADR を必要とする system-wide decision か、context-specific decision か
