...
ハーネス全体を起動するシェルコマンドはありません。Codexへ「ハーネスを使う」と明示して依頼すると、ユーザーのPCにある個人用skillが起動します。

IssueからPR作成まで進める場合は、次のように依頼します。

> Issue #123をハーネスでPRまで進めて

内部では、次の処理が順番に連携します。

```mermaid
flowchart LR
    U[ユーザーの依頼] --> I[issue-to-pr<br/>Issue・範囲・ブランチを固定]
    I --> H[review-remediation-harness<br/>初回レビュー・修正・試験・最終レビュー]
    H --> P[create-pr<br/>commit・push・PR作成]
    P --> M[ユーザー<br/>mergeを判断]
```

Issueを使わず、現在のブランチを対象にする場合は、次のように依頼します。

> このブランチをmainとの差分でハーネスにかけて。修正担当と最終レビュー担当を分け、PR作成まで進めて

通常の「Issue #123を進めて」という依頼だけでは、自動的にハーネスを起動しません。初期版では通常のIssue対応と区別するため、「ハーネスを使って」と明示された場合だけ起動します。

`review-harness-artifacts`はハーネス全体の起動コマンドではありません。作業記録の保存、検証、保存済みtargetの照合だけを担当する内部ツールです。
...
| `Introduced` | 今回の変更で新しく発生させた問題 | 新しく追加した処理に認可確認を書き忘れた |
| `Exposed` | 原因の一部は以前からあったが、今回の変更によって表面化、悪化、依存などで対応が必要になった問題 | 以前から危険だった関数を、新しい画面から呼び出せるようにした |
...
まとめると、`Introduced`と`Exposed`は「今回の変更によって対応が必要になった問題」です。一方、以前から存在し、今回の変更が到達性、影響、依存関係を変えていない問題は`Pre-existing`として別枠で報告します。ハーネスが`READY`と判断するには、`Introduced`または`Exposed`のCritical/Majorが0件でなければなりません。分類の正確な規則は[principle-of-programming-reviewerのOrigin](../../../shared/references/principle-of-programming-reviewer.md#origin)を正本とします。
