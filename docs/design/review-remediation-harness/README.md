...
「元のPRで変更されているファイル」と「依頼を満たすために修正してよい範囲」は別です。たとえば、bug修正に必要なテストや、既存仕様に合わせる運用手順は、元のPRに含まれていなくても修正候補になります。ただし、依頼との関係を説明できる最小変更に限り、ユーザーが明示した禁止事項は守ります。

進行役が自分で範囲を狭く設定しただけなら、そのミスを理由に毎回ユーザーへ確認しません。元の許可の範囲内と確認できた場合は、理由と履歴を残して訂正します。確定済みの変更可能なパス一覧を変える場合は、新しい実行記録へ引き継ぎますが、元の停止記録・期限・使用済み回数は引き継ぐため、上限はリセットされません。別機能の追加、仕様の選択、権限の追加が必要な場合や、許可を確認できない場合は引き続き停止します。
