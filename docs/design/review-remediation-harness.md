...
| `write_external_system` | false | create-pr phase担当 | 明示的なPR依頼のexact fetch/push transport locator、remote、同一であるbase/head repository identity、branch、SHAとcreate-pr metadata policyに限定。その他のexternal writeは許可しない |
