...
- `publish_exact_candidate`: READY済みcandidate SHAとbase SHA、exact fetch/push transport locator、同一であるremote/base/head canonical repository identity、`read_external_source`、`fetch_remote_refs`と設定済みrefspec/prune allowlistを入力にし、base fetch後の一致確認、exact head refのpre-write lookupと必要時のsource-only fetch、fixed push locatorへの`--no-verify`付きexact `<head_sha>:refs/heads/<head-ref>` push、PR作成または更新だけを行う。File編集、targetを変更し得る品質gate、stage、追加commitは禁止する。Fork PRはV1 capability不足として外部write前に停止する。
