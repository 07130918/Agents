...
- `Glob` で `~/.agents/skills/*/SKILL.md` と `~/.codex/agents/*.toml` を列挙
- `Read` で `~/.codex/AGENTS.md` を取得
- skill の frontmatter `description` と agent の `name` / `description` を並列で `Read` し、カバー範囲を一覧化
- `~/.claude/skills.disabled/` と `~/.claude/agents.disabled/` はCodexの有効な能力として数えない
...
```bash
find ~/.codex/sessions ~/.codex/archived_sessions -name "*.jsonl" -mtime -30 | sort
```
...
- `Grep(pattern: "Chakra|Next\\.js|Flask|FastAPI|Knex|MySQL|TypeScript|Supabase|Slack|Bitcoin|Figma|Asana", path: "~/.codex/sessions/", glob: "*.jsonl", output_mode: "count", head_limit: 30)`

**繰り返しのタスク種別** - 計画・修正・バグ・実装の頻出ワード:

- `Grep(pattern: "Chakra UI|マイグレーション|権限|RBAC|ダッシュボード|ログ|API|コードレビュー|PR作成|バグ|テスト", path: "~/.codex/sessions/", glob: "*.jsonl", output_mode: "count", head_limit: 20)`

**エラーパターン** - 直近2週間のセッションからサンプル:

1. Bash で対象 `.jsonl` を選定: `find ~/.codex/sessions ~/.codex/archived_sessions -name "*.jsonl" -mtime -14 | sort | tail -5`
...
3. **既存スキル・エージェントが最新の実態と乖離していないか**
   ...
   - skill だけでなく `~/.codex/agents/*.toml` の役割とhandoffも確認する
   - 削除済みskillの責務をactive agentが継承している場合は、skill再作成ではなく参照更新を候補にする
4. **AGENTS.md や既存 skill の肥大・重複**
   - 一次的な違和感レベルで留め、詳細な監査・リファクタは `audit-codex-config` に委譲する (二重実装を避ける)

### Codexレビュー能力の判定

- 汎用コードレビューはactiveな `pr-risk-reviewer` agentが担当する。
- `codex-review` はCodex用skillの作成候補、呼び出し先、代替案に含めない。
- 文書が `codex-review` を参照している場合は、不足skillとして再作成せず、`pr-risk-reviewer` への参照更新候補として報告する。
- `principle-of-programming-reviewer` は設計原則レビューに限定し、`pr-risk-reviewer` と重複扱いしない。
...
- 提案前に同じ責務がactiveなskillまたはagentに存在しないことを確認する。reference内の古い名前だけを根拠に、不足skillと判定しない
...
**新規スキル** `~/.agents/skills/{name}/SKILL.md`:
...
**新規エージェント** `~/.codex/agents/{name}.toml`:

```toml
name = "{name}"
description = "{役割、使う場面、対象外}"
