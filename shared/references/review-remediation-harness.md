...
11. PR提出が許可されていれば、`fetch_remote_refs`、push、PR、operation限定の`write_external_system` permissionを確認し、exact desired submissionのpublish intent checkpointを保存してから`publish_exact_candidate`だけを実行する。Read-back後にobservation checkpointを保存し、Defaultのmonolithic `create-pr`を再実行しない。
