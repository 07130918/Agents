...
- `write_external_system`: 初期false。明示的なPR作成または更新依頼がある場合だけ、`publish_exact_candidate`の対象remote、base/head repository identity、branch、SHAとcreate-pr metadata policyに限定してtrueにできる。
...
明示的なPR作成または更新依頼は、そのexact request、actor、run、remote、base/head repository identity、branch、SHA、create-pr標準templateと許可assignee/labelを含むmetadata policyをHuman input snapshotへ固定し、`push`、`create_or_update_pr`、このoperationだけの`write_external_system`を同じpermission setでtrueにするためのapprovalとして扱う。Exact `desired_submission`はこのpolicyとcandidate差分から外部write前に生成しintentで固定するが、その生成だけでpermission inputを差し替えない。この専用publishはresolved project commandの`external_write`例外ではなく、READY後に`external_operation`とcreate-pr contractでのみ実行できる。他のremote、branch、permission外PR metadata、Issue comment、SaaS operationへ承認を流用しない。
