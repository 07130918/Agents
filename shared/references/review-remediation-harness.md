...
`desired_submission`は`title`、exact `body`、`draft`、loginのUTF-8 byte順で重複ない`assignees`、label名の同順で重複ない`labels`を持つ。`desired_submission_sha256`はそのJCS bytesのSHA-256とする。`external_operation`の`intent`は外部call前に保存し、`intent_ref: null`、初回は`previous_observation_ref: null`、`result: pending`、全stepが`pending`の`step_results`、`external_effect_observed: false`、`remote_head_status: null`、`remote_head_sha: null`、`pr_identifier: null`、`observed_pr_state: null`、空の`read_back_evidence_refs`、`observed_at: null`にする。2回目以降のintentは直前observationのcommon refを`previous_observation_ref`へ持つ。

`operation_id`はexpected repository identity、base/head SHA、remote URL/name、base/head branch、operation kind、`desired_submission_sha256`のJCS valueをSHA-256化した`sha256-<小文字16進64文字>`、`execution_key`はoperation ID、attempt、permission set hashのJCS valueから同形式で作る。Observationは同じoperation/execution key、desired submissionと先行intent refを持ち、`result: succeeded|not_performed|partially_performed|unknown|ready_invalidated`、`push: pending|not_needed_exact|performed|not_performed|unknown|ready_invalidated`、`pull_request: pending|created|updated|not_performed|partial|unknown`、`read_back: pending|exact|mismatch|unknown`の`step_results`、外部effectの有無、`remote_head_status: absent|exact|ancestor|diverged_or_ahead|unresolved`、remote head、PR identifier、`observed_pr_state`、full evidence refs、`observed_at`を保存する。`observed_pr_state`は`{"status":"absent"}`、`{"status":"present","identifier":"<id>","state":"OPEN","base_branch":"<branch>","base_sha":"<oid>","head_branch":"<branch>","head_sha":"<oid>","title":"<exact>","body":"<exact>","draft":<boolean>,"assignees":[...],"labels":[...]}`、`{"status":"conflicting","identifiers":[...]}`、`{"status":"unresolved","reason":"<reason>"}`のunionとし、配列はdesired submissionと同じ順序、identifierはUTF-8 byte順にする。Remote head statusが`absent|unresolved`な場合は`remote_head_sha: null`、それ以外は観測したfull OIDを必須にし、再取得不能の理由をread-back evidenceに保存する。

`succeeded`はremote headがexpected head、PR stateが`present`、全desired fieldがexact、`read_back: exact`の場合だけとする。`not_performed`はremote head statusが`absent|ancestor`、PR stateが`absent`で、次のnon-force exact pushが安全な場合だけとする。`partially_performed`は後述の安全な中間状態だけに限定する。条件のどれかを証明できない場合は`unknown`とする。Intentを含むManifestとobservationを含むManifestはいずれも`previous_state: READY`、`state: READY`の非状態変更checkpointにする。ただし`ready_invalidated`は続けてinvalidation decisionを保存し`CONTEXT_RESOLVING`へ、`unknown`は自動retryせず`HUMAN_DECISION_REQUIRED`へ遷移する。

Resumeでobservationのないpublish intentを見つけた場合、同じoperationを再実行する前にremote headとopen PRのbase/head/title/body/draft/assignee/labelをread-backする。Remote headがexpected headと一致し、一意なopen PRの全fieldが`desired_submission`と一致する場合だけ`succeeded`を保存する。Remote head statusが`absent|ancestor`かつPRがabsentなら`not_performed`、remote headだけがexactまたは正しいbase/headの一意なPRの更新可能metadataだけが不一致なら`partially_performed`にする。後者はcreate-pr contractが保証するexact headのpush skipと、同一PRへのidempotentなtitle/body/assignee/label設定だけを、retry budget内の次attemptと新しいexecution keyで再開できる。Draft不一致、複数PR、異なるbase/head、diverged remote、または1 fieldでも再取得不能なら`unknown`または`ready_invalidated`とし、推測retryしない。
...
- `write_external_system`: 初期false。明示的なPR作成または更新依頼がある場合だけ、`publish_exact_candidate`の対象repository/remote/base/headとcreate-pr metadata policyに限定してtrueにできる。
