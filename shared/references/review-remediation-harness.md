...
`desired_submission`は`title`、exact `body`、`draft`、loginのUTF-8 byte順で重複ない`assignees`、label名の同順で重複ない`labels`を持つ。`desired_submission_sha256`はそのJCS bytesのSHA-256とする。`external_operation`の`intent`は外部call前に保存し、`intent_ref: null`、各runの初回は`previous_observation_ref: null`、`result: pending`、全stepが`pending`の`step_results`、`effect_provenance: {"push":"not_performed","pull_request":"not_performed"}`、`external_effect_observed: false`、`remote_head_status: null`、`remote_head_sha: null`、`pr_identifier: null`、`observed_pr_state: null`、空の`read_back_evidence_refs`、`observed_at: null`にする。Prior run handoffがある初回intentは、cross-run common refを作らず、self-contained handoff inputをenvelopeの`input_refs`へ含める。同じrunの2回目以降のintentだけが、直前observationのcommon refを`previous_observation_ref`へ持つ。
