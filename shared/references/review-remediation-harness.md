...
`BUDGET_EXHAUSTED`がpublishの未完了operation中に発生した場合、`prior_run_handoff.content`は上記に加えてoperation ID、desired submission hash、intentのartifact ID/hashとexact payload、最新observationのartifact ID/hashとexact payloadを通常refではない値としてcopyする。新runはremote/PRを再取得し、新しいpermissionとbudgetで別のoperation IDを作らず、同じdesired operationの安全な未完了stepだけを再開する。
...
| `external_operation` | `operation_kind: publish_exact_candidate`、`record_kind: intent|observation`、`operation_id`、`execution_key`、`intent_ref`、`previous_observation_ref`、`remote_repository_identity`、`expected_base_repository_identity`、`expected_head_repository_identity`、`expected_base_sha`、`expected_head_sha`、`remote_name`、`remote_url`、`base_branch`、`head_branch`、`desired_submission`、`desired_submission_sha256`、`attempt`、`result`、`step_results`、`external_effect_observed`、`remote_head_status`、`remote_head_sha`、`pr_identifier`、`observed_pr_state`、`read_back_evidence_refs`、`started_at`、`observed_at` |
...
- `fetch_remote_refs`: 初期false。CommitまたはPR依頼で明示されたcanonical repository identity、remote、refspec、prune範囲だけtrueにできる。
...
- `write_external_system`: 初期false。明示的なPR作成または更新依頼がある場合だけ、`publish_exact_candidate`の対象remote、同一であるbase/head canonical repository identity、branch、SHAとcreate-pr metadata policyに限定してtrueにできる。
