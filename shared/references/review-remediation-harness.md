...
- `read_external_source`: 明示source/hostだけtrue。Authorityは別判定。PR依頼ではcreate-prのidentity解決、same-repository open PR検索、post-write read-backに必要なGitHub API sourceだけを追加できる。
- `fetch_remote_refs`: 初期false。CommitまたはPR依頼で明示されたexact fetch transport locator、remote、refspec、prune範囲だけtrueにできる。GitHub API identityはprepare-only fetchの要件にしない。
