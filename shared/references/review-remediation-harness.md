...
`operation_id`はexpected base/head repository identity、base/head SHA、remote URL/name、base/head branch、operation kind、`desired_submission_sha256`のJCS valueをSHA-256化した`sha256-<小文字16進64文字>`、`execution_key`はrun ID、operation ID、run-local attempt、permission set hashのJCS valueから同形式で作る。新runへhandoffしたoperationは同じoperation IDを保つがattemptをrun-localに数え直し、run IDを含むexecution keyによって旧runのattemptと衝突させない。Observationは同じoperation/execution key、desired submissionと先行intent refを持ち、`result: succeeded|not_performed|partially_performed|unknown|ready_invalidated`、`push: pending|not_needed_exact|performed|not_performed|unknown|ready_invalidated`、`pull_request: pending|created|updated|not_performed|partial|unknown`、`read_back: pending|exact|mismatch|unknown`の`step_results`、外部effectの有無、`remote_head_status: absent|exact|ancestor|diverged_or_ahead|unresolved`、remote head、PR identifier、`observed_pr_state`、full evidence refs、`observed_at`を保存する。`observed_pr_state`は`{"status":"absent"}`、`{"status":"present","identifier":"<id>","state":"OPEN","base_repository_identity":"<identity>","base_branch":"<branch>","base_sha":"<oid>","head_repository_identity":"<identity>","head_branch":"<branch>","head_sha":"<oid>","title":"<exact>","body":"<exact>","draft":<boolean>,"assignees":[...],"labels":[...]}`、`{"status":"conflicting","identifiers":[...]}`、`{"status":"unresolved","reason":"<reason>"}`のunionとし、配列はdesired submissionと同じ順序、identifierはUTF-8 byte順にする。Remote head statusが`absent|unresolved`な場合は`remote_head_sha: null`、それ以外は観測したfull OIDを必須にし、再取得不能の理由をread-back evidenceに保存する。

`succeeded`はremote headがexpected head、PR stateが`present`、base/head repository identity、branch、SHAと全desired fieldがexact、`read_back: exact`の場合だけとする。`not_performed`はremote head statusが`absent|ancestor`、PR stateが`absent`で、次のnon-force exact pushが安全な場合だけとする。`partially_performed`は後述の安全な中間状態だけに限定する。条件のどれかを証明できない場合は`unknown`とする。Intentを含むManifestとobservationを含むManifestはいずれも`previous_state: READY`、`state: READY`の非状態変更checkpointにする。ただし`ready_invalidated`は続けてinvalidation decisionを保存し`CONTEXT_RESOLVING`へ、`unknown`は自動retryせず`HUMAN_DECISION_REQUIRED`へ遷移する。

Resumeでobservationのないpublish intentを見つけた場合、同じoperationを再実行する前にremote headとopen PRのbase/head repository identity、branch、SHA、title、body、draft、assignee、labelをread-backする。Remote headがexpected headと一致し、一意なopen PRの全fieldがexpected identitiesと`desired_submission`に一致する場合だけ`succeeded`を保存する。Remote head statusが`absent|ancestor`かつPRがabsentなら`not_performed`、remote headだけがexactまたは正しいbase/head identities、branch、SHAの一意なPRの更新可能metadataだけが不一致なら`partially_performed`にする。後者はcreate-pr contractが保証するexact headのpush skipと、同一PRへのidempotentなtitle/body/assignee/label設定だけを、retry budget内の次attemptと新しいexecution keyで再開できる。Draft不一致、複数PR、base/head repository identity、branch、SHAのいずれかが異なるPR、diverged remote、または1 fieldでも再取得不能なら`unknown`または`ready_invalidated`とし、推測retryしない。
...
External write attempt counterは専用`publish_exact_candidate`の外部call前だけ予約する。実行済み範囲は`external_operation` observationでread-backし、上記の`not_performed|partially_performed`条件を満たす場合だけ次attemptを開始できる。成功不明のtimeoutは自動retryせず、read-backで確定できなければ`HUMAN_DECISION_REQUIRED`にする。
...
- `PARTIALLY_PERFORMED`: create-prのphase result。Manifest stateではない。Exact targetを保った安全な中間状態とstep別read-backをobservationへ保存し、READYを失効させずbudget内の次attemptまたは`BUDGET_EXHAUSTED`を選ぶ。
